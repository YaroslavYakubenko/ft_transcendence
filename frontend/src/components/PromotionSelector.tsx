...
type Props = {
	...
	do_promotion: (fen: string, move: string, promo: string, gameId?: number | null) => Promise<PromotionResponse | null>
	gameId?: number | null
}
							...
							onClick={() => {
								do_promotion(fen, promotion.move, promo, gameId).then((data) => {
									...
									setMoves((prevMoves) => {
										...
										if ((data as any).bot_move) {
											nextMoves = appendMove(nextMoves, (data as any).bot_move, !isWhiteMove)
										}
										return nextMoves
									})
