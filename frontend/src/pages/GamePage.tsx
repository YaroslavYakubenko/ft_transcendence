...
function GamePage() {
	...
	// game vars
	const [fen, setFen] = useState(() => loadFen(storage_keys.fen, location.state?.rematchId))
	const [moves, setMoves] = useState<{ white: string; black?: string }[]>(() => loadMoves(storage_keys.move_history))
	const [result, setRes] = useState(() => loadResult(storage_keys.result))
	...
	const sendMove = async (from: string, to: string, currentFen: string) => {
		if (isLive && wsRef.current?.readyState === WebSocket.OPEN) {
			...
			return null // state will be updated via onmessage
		}
